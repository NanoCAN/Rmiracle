...
RUN mkdir /rmiracle
ADD . /rmiracle
RUN R -e "install.packages('devtools');devtools::install_local('/rmiracle')"
...
RUN sed -i 's/site_dir \/srv\/shiny-server;/app_dir \/srv\/rmiracle;/g' /etc/shiny-server/shiny-server.conf
