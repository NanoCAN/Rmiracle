...
shinyServer(function(input, output, session) {
  ...
  #load the slide from MIRACLE
  getSlide <- function(baseUrl, id){
    rppa.load(baseUrl=baseUrl, slideIndex=id)
  }  
  
  #save a local copy of the slide object, so we don't have to reload it all the time
  currentSlide <- reactive({
    #decode the URL parameters
    query <- parseQueryString(session$clientData$url_search)
    #if we don't have a URL we assume it's localhost
    if(length(query$baseUrl) > 0) baseUrl <- query$baseUrl
    else baseUrl <- "http://localhost:8080/MIRACLE/spotExport/"
    getSlide(baseUrl, query$id)
  })
